...
func (suite *KeeperSuite) TestSetCollection() {
	nft := types.NewBaseNFT(tokenID, tokenNm, address, tokenURI, true)
	// create a new NFT and add it to the collection created with the NFT mint
	nft2 := types.NewBaseNFT(tokenID2, tokenNm, address, tokenURI, true)
...
func (suite *KeeperSuite) TestGetSupply() {
	...
	// MintNFT shouldn't fail when collection does not exist
	err = suite.keeper.MintNFT(suite.ctx, denomID2, tokenID, tokenNm2, tokenURI, address, address2, true)
	suite.NoError(err)
