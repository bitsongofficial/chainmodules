...
func (suite *KeeperSuite) TestGetNFT() {
	...
	err = suite.keeper.MintNFT(suite.ctx, denomID, tokenID2, tokenNm2, tokenURI, address, address, true)
	suite.NoError(err)
...
func (suite *KeeperSuite) TestGetNFTs() {
	...
	err = suite.keeper.MintNFT(suite.ctx, denomID2, tokenID2, tokenNm2, tokenURI, address, address, true)
	suite.NoError(err)

	err = suite.keeper.MintNFT(suite.ctx, denomID2, tokenID3, tokenNm3, tokenURI, address, address, true)
	suite.NoError(err)

	err = suite.keeper.MintNFT(suite.ctx, denomID, tokenID3, tokenNm3, tokenURI, address, address, true)
	suite.NoError(err)
